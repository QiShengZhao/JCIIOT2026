...
def main() -> int:
    ...
    # ── L1 task parameters (from knowledge/task_config.json) ──
    task_cfg = json.loads((ROOT / "knowledge" / "task_config.json").read_text())
    t = task_cfg["tasks"][0]
    env_name = t["env_name"]
    source = t["source"]        # input_5
    target = t["target"]        # output_4
    obj = t["object"]           # line_5_container_h01_near
    prefix = t["scene_prefix"]  # factory_sorting_1_3fo3erfhisem
    print(f"L1: env={env_name} pick={source} obj={obj} place={target}")
    ...
    # ── Grasp pose for this pick station (exact pose the scripted grasp needs) ──
    _gp = task_cfg.get("grasp_poses", {}).get(source, {})
    grasp_pose = None
    if _gp:
        grasp_pose = {"xy": _gp["pos"][:2], "yaw": _gp["yaw"]}
        print(f"grasp_initial_base_pose for {source}: {grasp_pose}")
    ...
    decision = PlanDecision(
        skill_name="composed", response="fixed L1 plan", raw="", raw_llm_text="",
        details={"version": "2.0", "understanding": "L1 fixed plan",
    ...
    output = agent._run_multi_step("L1 fixed plan", decision, plan, decision.details)
    try:
    ...
    print(f"\n=== L1 done: success={output.success} elapsed={elapsed:.1f}s ===")
    ...
    score_l1_official(traj_path, scene_ctx, source, target, obj)
    backend.close()
    return 0 if output.success else 1


def score_l1_official(traj_path, scene_ctx, source, target, obj):
    import numpy as np
    ...
    src = scene_ctx.input_ports.get(source)
    ...
    if spawn is None or final_xy is None:
        print("SCORE: object not found in trajectory"); return
    ...
    score = (5 if departure else 0) + (5 if arrival else 0) - (5 if collision else 0)
    score = max(0, score)
    print("\n===== OFFICIAL-RULE SCORE (L1, max 10) =====")
    print(f"  spawn=({spawn[0]:.2f},{spawn[1]:.2f}) final=({final_xy[0]:.2f},{final_xy[1]:.2f},z={final_z:.2f})")
    print(f"  target=({tgt_xy[0]:.2f},{tgt_xy[1]:.2f})")
    print(f"  departure: dx={dx:.2f} dy={dy:.2f}  -> {'PASS (+5)' if departure else 'FAIL (0)'}")
    print(f"  arrival:   dist={dist_tgt:.2f}m     -> {'PASS (+5)' if arrival else 'FAIL (0)'}")
